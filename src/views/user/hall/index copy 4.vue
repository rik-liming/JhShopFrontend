<template>
  <div>
    ...
    <div class="app-container" @scroll="onScroll">
      ...
      <!-- 表格容器 -->
      <div class="table-wrapper" style="position: relative; overflow: hidden; width: 100%;">
        <!-- 旧表格 -->
        <el-table
          ...
          style="width: 100%; position: absolute; top: 0; left: 0; transition: transform 0.5s ease-in-out;"
          :key="'table-' + tableKey"
        ...
        <el-table
          ...
          fit
          highlight-current-row
          style="width: 100%; position: absolute; top: 0; left: 100%; transition: transform 0.5s ease-in-out;"
          :key="'table-new-' + tableKey"
          @sort-change="sortChange"
        >
          <el-table-column label="ID" prop="id" sortable="custom" align="center" width="80" :class-name="getSortClass('id')">
            <template v-slot="{ row }">
...
.table-wrapper-enter-active, .table-wrapper-leave-active {
  transition: transform 0.5s ease-in-out;
}

.table-wrapper-enter, .table-wrapper-leave-to {
  transform: translateX(100%); /* 新表格从左侧滑入 */
}
