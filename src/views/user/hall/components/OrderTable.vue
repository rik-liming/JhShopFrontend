...
const handleRowClick = (row) => {
  ...
  if (role === 'buyer' || role === 'autoBuyer') {
    // 根据点击的行的数据，构造目标路由地址
    targetPage = `/order/buyer/detail?orderId=${row.id}&goback=/hall&myTableType=order`; // 假设根据 row.id 构造跳转路径
    router.push(targetPage); // 跳转到 /buy 页面，带上 tradeId 参数
  } else if (role === 'seller' || role === 'agent') {
    targetPage = `/order/seller/detail?orderId=${row.id}&goback=/hall&myTableType=order`; // 假设根据 row.id 构造跳转路径
    router.push(targetPage); // 跳转到 /buy 页面，带上 tradeId 参数
  }
};
